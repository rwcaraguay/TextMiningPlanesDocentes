...
shinyServer(function(input, output) {
  output$savecontenido = renderPrint({
    ...
    if (is.null(cont))
      return(NULL)
    write.table(x = cont, "Prueba1/corpus/plan.txt")
  })
  ...
  output$mtd = renderTable({
    ...
    limpieza = isolate({
      ...
      #Eliminar palabras vacías a partir de un documento
      ...
      writeCorpus(corpusmatriz, path = "Prueba1/preprocesados/") #guardar los archivos pre-procesados
      
      vocabulario = readLines("Prueba1/archivos/vocabulario", encoding = "UTF-8")#Cargar archivo con el vocabulario
      ...
      write.csv(matriz, file = "Prueba1/resultados/mtdp1", row.names = TRUE, fileEncoding = "ASCII//TRANSLIT")#guardar la mtd en formato csv  
      matriz
    })
    limpieza
  })
  ...
  output$plotkmeans = renderPlot({
    ...
    algoritmokmeans = isolate({
      ...
      write.csv(dataframe,file = "Prueba1/resultados/clustersprueba1.csv")
    })
    algoritmokmeans
  })
  ...
  output$plotsvd = renderPlot({
    ...
    svdmatriz = isolate({
      ...
      vocabulario = iconv(vocabulario, to = "ASCII//TRANSLIT")
      matrizterminodocumento = DocumentTermMatrix(corpusmatriz, control = list(dictionary = c(vocabulario), tokenize = NGramTokenizer)) #Crear MTD apartir del vocabulario
      matriz = as.matrix(matrizterminodocumento)#Crear matriz de caracteres apartir de los térrminos
      write.csv(matriz, file = "Prueba1/resultados/mtdp1", row.names = TRUE, fileEncoding = "ASCII//TRANSLIT")#guardar la mtd en formato csv  
      
      matrizsvd = matriz #Matriz término documento
      algsvd =  svd(matrizsvd) #Algoritmo SVD
      matrizu <- algsvd$u[,1:3] #Reducción de la dimensión del espacio
      plot(matrizu, asp=1, main = "Representación de los documentos en el plano", pch=16, col=34);
      abline(h = 0, v = 0, lty =5, col=28) #Gráfica de los documentos
