...
shinyServer(function(input, output) {
  ...
  output$savecontenido = renderPrint({
    ...
    if (is.null(cont))
      return(NULL)
    write.table(x = cont, "Prueba2/corpus/pn2.txt")
  })
  ...
  output$mtd = renderTable({
    ...
    limpieza = isolate({
      documentos = Corpus(DirSource("Prueba2/corpus/", encoding = "UTF-8"))
      ...
      writeCorpus(corpusmatriz, path = "Prueba2/preprocesados/") #guardar los archivos pre-procesados
      
      vocabulario = readLines("Prueba2/archivos/vocabulariop2", encoding = "UTF-8")#Cargar archivo con el vocabulario
      ...
      write.csv(matriz, file = "Prueba2/resultados/mtdp2", row.names = TRUE, fileEncoding = "ASCII//TRANSLIT")#guardar la mtd en formato csv  
      matriz
    })
    limpieza
  })
  ...
  output$plotkmeans = renderPlot({
    ...
    algoritmokmeans = isolate({
      ...
      write.csv(dataframe,file = "Prueba2/resultados/clusters-prueba2.csv")
    })
    algoritmokmeans
  })
