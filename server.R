...
shinyServer(function(input, output) {
    ...
    if (is.null(cont))
      return(NULL)
    write.table(x = cont, "/home/robert/R/AppTextMining/TextMining/pruebas/prueba1/corpus/plan.txt")
  })
  ...
  output$mtd = renderTable({
    ...
    limpieza = isolate({
      ...
      write.csv(matriz, file = "/home/robert/R/AppTextMining/mtdp1.csv", row.names = TRUE, fileEncoding = "ASCII//TRANSLIT")#guardar la mtd en formato csv  
      matriz
    })
    limpieza
  })
  ...
  output$plotkmeans = renderPlot({
    ...
    write.csv(dataframe,file = "clusters.csv")
    })
    algoritmokmeans
  })
