                        ...
                        navbarPage("", class="navbar navbar-default",
                                   tabPanel("Inicio",
                                            ...
                                            tags$blockquote(
                                              sidebarLayout(
                                                sidebarPanel(
                                                  numericInput(inputId = "numclusters", label = "Número de grupos", value = 6, min = 1, max = 10, width = "50%"),
...
server <- shinyServer(function(input, output) {
  output$savecontenido = renderPrint({
    ...
    if (is.null(cont))
      return(NULL)
    write.table(x = cont, "Prueba3/corpus/pdn")
  })
  ...
  output$mtd = renderTable({
    ...
    limpieza = isolate({
      documentos = Corpus(DirSource("Prueba3/corpus/", encoding = "UTF-8"))
      ...
      writeCorpus(corpusmatriz, path = "Prueba3/preprocesados/") #guardar los archivos pre-procesados
      
      vocabulario = readLines("Prueba3/archivos/vocabulariop3", encoding = "UTF-8")#Cargar archivo con el vocabulario
      ...
      write.csv(matriz, file = "Prueba3/resultados/mtdp3.csv", row.names = TRUE, fileEncoding = "ASCII//TRANSLIT")#guardar la mtd en formato csv  
      matriz
    })
    limpieza
  })
  ...
  output$plotkmeans = renderPlot({
    ...
    algoritmokmeans = isolate({
      ...
      write.csv(dataframe,file = "Prueba3/resultados/clusters-prueba3.csv")
    })
    algoritmokmeans
  })
  ...
  output$plotsvd = renderPlot({
    ...
    svdmatriz = isolate({
      ...
      matrizterminodocumento = DocumentTermMatrix(corpusmatriz, control = list(dictionary = c(vocabulario), tokenize = NGramTokenizer)) #Crear MTD apartir del vocabulario
      matriz = as.matrix(matrizterminodocumento)#Crear matriz de caracteres apartir de los t?rminos
      write.csv(matriz, file = "Prueba3/resultados/mtdp3", row.names = TRUE, fileEncoding = "ASCII//TRANSLIT")#guardar la mtd en formato csv  
      
      matrizsvd = matriz #Matriz término documento
      algsvd =  svd(matrizsvd) #Algoritmo SVD
      matrizu <- algsvd$u[,1:6] #Reducción de la dimensión del espacio
