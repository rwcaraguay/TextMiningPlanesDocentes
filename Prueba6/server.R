...
library(stats) #Libreria para el K-means
...
shinyServer(function(input, output) {
  ...
  output$savecontenido = renderPrint({
    ...
    if (is.null(cont))
      return(NULL)
    write.table(x = cont, "Prueba6/corpus/pd")
  })
  ...
  output$mtd = renderTable({
    ...
    limpieza = isolate({
      documentos = Corpus(DirSource("Prueba6/corpus/", encoding = "UTF-8"))
      ...
      writeCorpus(corpusmatriz, path = "Prueba6/preprocesados/") #guardar los archivos pre-procesados
      
      vocabulario = readLines("Prueba6/archivos/vocabulariop6", encoding = "UTF-8")#Cargar archivo con el vocabulario
      ...
      write.csv(matriz, file = "Prueba6/resultados/mtdp6.csv", row.names = TRUE, fileEncoding = "ASCII//TRANSLIT")#guardar la mtd en formato csv  
      matriz
    })
    limpieza
  })
  ...
  output$plotkmeans = renderPlot({
    ...
    algoritmokmeans = isolate({
      ...
      write.csv(dataframe,file = "Prueba6/resultados/clusters-prueba6.csv")
    })
    algoritmokmeans
  })
